...
# 경력 수치형 데이터 변환
df["경력"]=df["경력"].str.replace("경력","").str.replace("\(최소","").str.replace("\)","").str.replace("이상","").str.replace("우대","").str.replace("필수","").str.replace("0개월","").str.replace("개월","").str.replace("년",".").apply(lambda x: float(x))
